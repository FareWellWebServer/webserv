...
void Parse_Req_Msg(int c_socket, ReqHandler& reqhandle) {
  ...
  //------------body parsing-----------------------
  reqhandle.req_msg_.body_data_.entity_ = strdup("");

  char* buf = new char[reqhandle.req_msg_.body_data_.entity_length_ + 1];
  int byte;

  ctmp = get_next_line(c_socket, 1);
  std::cout << ctmp << std::endl;
  byte = read(c_socket, buf, reqhandle.req_msg_.body_data_.entity_length_);
  if (byte == 0)
    return;  // 종료
  else if (byte > 0) {
    buf[byte] = 0;
  ...
  //      i++) {
  //   // TODO : 0000이 들어가는 경우 문제가 생길수도. char buffer로
  //   한번에받아서
  //   // 처리.
